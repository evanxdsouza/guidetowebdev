...
async function init() {
  try {
    ...
    navList.innerHTML = `<p class="error">Couldn't load guides/manifest.json.<br>${err.message}</p>`;
...
async function loadFromHash() {
  ...
  if (!file) {
    contentEl.innerHTML = `<p class="error">No guides found. Add one to guides/ and list it in guides/manifest.json.</p>`;
    return;
  }
  ...
  try {
    ...
    contentEl.innerHTML = `<p class="error">Couldn't load guides/${file}.<br>${err.message}</p>`;
  }
}
